...
#	# Uses the 
#	def __init__(self):
#		self.N_pairs = 75 # no. of E/I pairs to a side of a grid
...
#		[self.W_EE, self.W_IE, self.W_EI, self.W_II] = generate_connetion_weights( self.N_pairs, self.field_size, self.OP_map, self.kappa_E, self.kappa_I, self.J_EE, self.J_IE, self.J_EI, self.J_II, self.sig_EE, self.sig_IE, self.sig_EI, self.sig_II, self.sig_ori )

#		self.sum_field_sizes_E = np.zeros((self.N_pairs, self.N_pairs)) 
#		self.sum_field_sizes_I = np.zeros((self.N_pairs, self.N_pairs)) 

#	def run_simulation(self, c, h, init_cond=np.zeros( (2, self.N_pairs, self.N_pairs) ) ):
...
#	    return [r_E, r_I, I_E, I_I]
  ...
  #   def plot_network_contrast_response(self, r_units=np.floor( self.N_pairs*np.random.rand(25,2) ), c_range=np.linspace(3, 50, 12) ):
		# pass

def load_OP_map(filename):
	pass

#def load_OD_map(filename='ocular-dom-obermayer-fig1.png'):
...
def generate_connetion_weights( N_pairs, field_size, OP_map, kappa_E, kappa_I, J_EE, J_IE, J_EI, J_II, sig_EE, sig_IE, sig_EI, sig_II, sig_ori ):
