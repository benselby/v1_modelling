...
class SSNetwork:
    ...
    def __init__(self, sig_EE=8, sig_IE=12, sig_EI=4, sig_II=4, J_EE=0.1, J_IE=0.38, J_EI=0.089, J_II=0.096, ori_map=0, ocd_map=None, od_bias=0., N_pairs=75, field_size=16.):
        ...
        self.r_E = theano.shared(np.zeros((75,75), dtype='float32'))
        self.r_I = theano.shared(np.zeros((75,75), dtype='float32'))
        self.increment_simulation = generate_theano_simulation(self.r_E, self.r_I)
    ...
    def run_simulation(self, c0, h0, timesteps=100, dt0=0.005):
        ...
        for t in range(timesteps):
            self.increment_simulation(dt0,c0,h0,self.W_EE,self.W_EI,self.W_IE,self.W_II,self.n_E,self.n_I,self.k,self.tau_E,self.tau_I)
...
def generate_theano_simulation(r_E, r_I):
