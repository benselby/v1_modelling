...
params = [1.2423    1.2336    1.9165    5.7963   86.8233  163.2025   -0.5580    1.9994];
...
lb = [0  1 .00001  -100 1];
ub = [1 10 200    100 1000]; 

numIter = 1;
xmulti = zeros(numIter,5);

paramsInitial = [1 1 50 0 1];
problem = createOptimProblem('lsqcurvefit','x0',paramsInitial,'objective',@LIF_contrast_response_no_gabor,...
...
estimate = LIF_contrast_response_no_gabor(neural_params, contrast);
figure();
...
lif_params = [neural_params(3) neural_params(4) neural_params(5)];
