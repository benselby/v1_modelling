...
lb = [1   1   0      -5  1 .0001  -100  1   1  2];
ub = [10 6  pi/180   5  10  500   100 10 4 2.5]; 

numIter = 1;
xmulti = zeros(numIter,10);

paramsInitial = [1 1 0 0 4 10 0 1 1 2 ];
problem = createOptimProblem('lsqcurvefit','x0',paramsInitial,'objective',@LIF_ori_contrast_response,...
...
estimate = LIF_ori_contrast_response(neural_params, contrast);
figure();
...
% figure()
% mesh(contrast, orientation, J_ori_contrast_response(neural_params, contrast))
% title('Current only')
% 
% figure()
